...
if (isset($_POST) && !empty($_POST)) {
    ...
    foreach($datas_rayon as $data_rayon) {
        if(isset($_POST['rayon_'.$data_rayon['id']])) {
            $h_rayon = array();
            $h_rayon['fk_produit'] = $id_produit;
            $h_rayon['fk_rayon'] = $data_rayon['id'];
...
$html .= '<li>Utilisateur: <strong>'. $_SESSION[SESSION_NAME]['nom'] .'</strong></li>';
$html .= '<li>';
...
if($datas_promotion){
    foreach($datas_promotion as $data_promotion) {
        $html .= '<option value="'.$data_promotion['id'].'">'.$data_promotion['nom'].'</option>';
    }
} else {
    $html .= '<option value="0">Aucune Promotion</option>';
}
$html .= '</select>';
$html .= '</div>';
$html .= '<div class="formField">';
$html .= '<label for="debut_promotion">Début Promotion</label>';
$html .= '<input type="date" name="debut_promotion" id="debut_promotion" placeholder="Début Promotion" value="'.date('Y-m-d',$data_produit['date_debut_promotion']).'"/>';
$html .= '</div>';
$html .= '<div class="formField">';
$html .= '<label for="fin_promotion">Fin Promotion</label>';
$html .= '<input type="date" name="fin_promotion" id="fin_promotion" placeholder="Fin Promotion" value="'.date('Y-m-d',$data_produit['date_fin_promotion']).'" />';
$html .= '</div>';
$html .= '</div>';
...
$datas_rayon = $bdd->getData($sql);
$html .= '<label for="rayon">Rayon</label>';
